    ...
    with st.form("respond"):
        ...
        if submit_button:
            ...
            if not customer_message and not audio_input:
                ...
                st.stop()
            try:
                payload = {"message": customer_message or None}
                if audio_input:
                ...
                response.raise_for_status()
                data = response.json()
                # Use transcription if available (from audio input), otherwise use customer_message
                customer_input = data.get("transcription") or customer_message or "Audio input (no transcription)"
                st.session_state.conversation.append({"role": "customer", "message": customer_input})
                ...
                if data.get("transcription"):
                    st.markdown(f"**Transcribed Audio**: {data['transcription']}")
...
if st.session_state.call_id:
    ...
    if st.session_state.sentiment_trend and len(st.session_state.sentiment_trend) > 1:
        st.write("#### Purchase Intent Analysis")
        intent_chart_data = {
            "type": "bar",
            "data": {
                "labels": [f"Turn {i+1}" for i in range(len(st.session_state.sentiment_trend))],
                "datasets": [{
                    ...
                    "data": st.session_state.sentiment_trend,
                    "backgroundColor": [
                        "#4CAF50" if x > 0 else "#F44336" if x < 0 else "#B0BEC5"
                        for x in st.session_state.sentiment_trend
                    ]
                }]
            },
            "options": {
                "scales": {
                    "y": {
                        "beginAtZero": True,
                        "title": {"display": True, "text": "Intent Score"},
                        "ticks": {"stepSize": 1}
                    },
                    "x": {
                        "title": {"display": True, "text": "Conversation Turn"}
        ...
        st.components.v1.html(f"""
            ...
            <script>
                ...
                new Chart(ctxIntent, {json.dumps(intent_chart_data)});
            </script>
        """, height=300)